...
public interface UserService {
    ...
    boolean showOneUser(int user_id, String password);//判断是否登录的账户信息是否正确

    boolean registerUser(int user_id, String username, String password) ;//判断是否注册，没有就注册

    List<BookMessage> showSelfBookMsg(int user_id);//用于查看自己书本的信息

    List<BookMessage> queryPublishedBookMsg(int user_id);//用于查看自己已经发布的书信息
    ...
    List<User> UserMessage(int user_id);
}
